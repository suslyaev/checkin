from django.core.management.base import BaseCommand
from django.contrib.auth.models import Group, Permission

class Command(BaseCommand):
    help = "Создаёт или обновляет базовые группы (Администратор, Менеджер, Проверяющий) с нужными правами."

    def handle(self, *args, **options):
        # Определяем набор разрешений для каждой группы
        # Список 'codename' можно посмотреть в админке -> Permissions или в базе
        group_specs = {
                'Администратор': ['add_action',
                            'change_action',
                            'delete_action',
                            'view_action',
                            'view_actionlog',
                            'add_categorycontact',
                            'change_categorycontact',
                            'delete_categorycontact',
                            'view_categorycontact',
                            'add_companycontact',
                            'change_companycontact',
                            'delete_companycontact',
                            'view_companycontact',
                            'add_contact',
                            'change_contact',
                            'delete_contact',
                            'view_contact',
                            'change_contactduplicateconflict',
                            'delete_contactduplicateconflict',
                            'view_contactduplicateconflict',
                            'add_customuser',
                            'change_customuser',
                            'delete_customuser',
                            'view_customuser',
                            'add_infocontact',
                            'change_infocontact',
                            'delete_infocontact',
                            'view_infocontact',
                            'add_community',
                            'change_community',
                            'delete_community',
                            'view_community',
                            'add_communitymember',
                            'change_communitymember',
                            'delete_communitymember',
                            'view_communitymember',
                            'add_moduleinstance',
                            'change_moduleinstance',
                            'delete_moduleinstance',
                            'view_moduleinstance',
                            'add_socialnetwork',
                            'change_socialnetwork',
                            'delete_socialnetwork',
                            'view_socialnetwork',
                            'add_typeguestcontact',
                            'change_typeguestcontact',
                            'delete_typeguestcontact',
                            'view_typeguestcontact',
                            'add_knownfirstname',
                            'change_knownfirstname',
                            'delete_knownfirstname',
                            'view_knownfirstname',
                            'add_knownlastname',
                            'change_knownlastname',
                            'delete_knownlastname',
                            'view_knownlastname'],
                'Менеджер': ['add_categorycontact',
                            'change_categorycontact',
                            'view_categorycontact',
                            'add_action',
                            'change_action',
                            'view_action',
                            'view_actionlog',
                            'add_companycontact',
                            'change_companycontact',
                            'view_companycontact',
                            'add_contact',
                            'change_contact',
                            'view_contact',
                            'change_contactduplicateconflict',
                            'view_contactduplicateconflict',
                            'view_customuser',
                            'add_infocontact',
                            'change_infocontact',
                            'delete_infocontact',
                            'view_infocontact',
                            'add_community',
                            'change_community',
                            'view_community',
                            'add_communitymember',
                            'change_communitymember',
                            'view_communitymember',
                            'add_moduleinstance',
                            'change_moduleinstance',
                            'view_moduleinstance',
                            'add_socialnetwork',
                            'change_socialnetwork',
                            'view_socialnetwork',
                            'add_typeguestcontact',
                            'change_typeguestcontact',
                            'view_typeguestcontact',
                            'add_knownfirstname',
                            'change_knownfirstname',
                            'view_knownfirstname',
                            'add_knownlastname',
                            'change_knownlastname',
                            'view_knownlastname'],
                'Модератор': ['view_categorycontact',
                            'add_action',
                            'view_action',
                            'view_actionlog',
                            'view_companycontact',
                            'view_contact',
                            'view_contactduplicateconflict',
                            'add_infocontact',
                            'view_infocontact',
                            'view_community',
                            'view_communitymember',
                            'view_moduleinstance',
                            'view_socialnetwork',
                            'view_typeguestcontact',
                            'view_knownfirstname',
                            'view_knownlastname'],
                'Продюсер': ['add_categorycontact',
                            'change_categorycontact',
                            'view_categorycontact',
                            'add_action',
                            'change_action',
                            'view_action',
                            'view_actionlog',
                            'add_companycontact',
                            'change_companycontact',
                            'view_companycontact',
                            'add_contact',
                            'change_contact',
                            'view_contact',
                            'change_contactduplicateconflict',
                            'view_contactduplicateconflict',
                            'view_customuser',
                            'add_infocontact',
                            'change_infocontact',
                            'delete_infocontact',
                            'view_infocontact',
                            'view_community',
                            'view_communitymember',
                            'view_moduleinstance',
                            'add_socialnetwork',
                            'change_socialnetwork',
                            'view_socialnetwork',
                            'add_typeguestcontact',
                            'change_typeguestcontact',
                            'view_typeguestcontact',
                            'add_knownfirstname',
                            'change_knownfirstname',
                            'view_knownfirstname',
                            'add_knownlastname',
                            'change_knownlastname',
                            'view_knownlastname']}

        for group_name, permission_codenames in group_specs.items():
            # Создаём или получаем группу по имени
            group, created = Group.objects.get_or_create(name=group_name)

            # Находим разрешения по списку codename
            perms = Permission.objects.filter(codename__in=permission_codenames)
            # Привязываем разрешения к группе
            group.permissions.set(perms)
            group.save()

            if created:
                self.stdout.write(self.style.SUCCESS(f"Группа '{group_name}' создана."))
            else:
                self.stdout.write(self.style.WARNING(f"Группа '{group_name}' обновлена."))

        self.stdout.write(self.style.SUCCESS("Группы успешно созданы/обновлены!"))